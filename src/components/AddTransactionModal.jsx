...
function dateToISO(dateStr) {
  // Evita el bug de timezone — interpreta la fecha en hora local, no UTC
...
export default function AddTransactionModal({ onClose, initial }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (initial?.id) {
        editTransaction(initial.id, txData)
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
      ...
      <div className="relative w-full max-w-md glass border border-white/10 rounded-t-3xl sm:rounded-3xl p-5 sm:p-6 shadow-2xl animate-slide-up"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Descripción */}
          <div>
            <label className="block text-[10px] uppercase tracking-widest text-white/30 mb-1.5">Descripción <span className="normal-case text-white/20">(opcional)</span></label>
